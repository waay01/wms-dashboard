            ...
            <defs>
              {levels.map(l => (
                <linearGradient key={l} id={`grad-${l}`} x1="0" y1="0" x2="0" y2="1">
                  <stop offset="5%" stopColor={LEVEL_COLORS[l] || '#94a3b8'} stopOpacity={0.3} />
                  <stop offset="95%" stopColor={LEVEL_COLORS[l] || '#94a3b8'} stopOpacity={0} />
                </linearGradient>
              ))}
            </defs>
            <CartesianGrid strokeDasharray="3 3" stroke="#1e293b" />
            <XAxis dataKey="time" tick={{ fontSize: 10, fill: '#475569', fontFamily: 'JetBrains Mono' }}
              tickFormatter={v => v ? new Date(v).toLocaleTimeString('ru', { hour: '2-digit', minute: '2-digit' }) : ''} />
            <YAxis tick={{ fontSize: 10, fill: '#475569', fontFamily: 'JetBrains Mono' }} />
            <Tooltip
              contentStyle={{ background: '#0f172a', border: '1px solid #1e3a5f', borderRadius: 8, fontFamily: 'JetBrains Mono', fontSize: 12 }}
              labelFormatter={v => new Date(v).toLocaleString('ru')}
            />
            <Legend wrapperStyle={{ fontSize: 11, fontFamily: 'JetBrains Mono' }} />
            {levels.map(l => (
              <Area key={l} type="monotone" dataKey={l}
                stroke={LEVEL_COLORS[l] || '#94a3b8'}
                fill={`url(#grad-${l})`}
                strokeWidth={1.5}
              />
            ))}
